; scarlet note: this has been nearly completely untouched from the original code present in the color hack except whereas noted
...
DeterminePaletteID: ; 71f97 (1c:5f97)
	...
	ld a, PAL_PURPLEMON      ; if yes, use Ditto's palette
	ret nz
	ld a, [hl]
...
DeterminePaletteIDNoShinyCheck:
; fixes a problem where viewing a shiny pokemon's status screen and then viewing the dex allows you to see all of the pokemon as shiny
	ld [wd11e], a
	and a
	jr z, .idZero
	push bc
...
DetermineBackSpritePaletteID: ; 71f97 (1c:5f97)
	bit 3, a                 ; bit 3 of battle status 3, set if current Pokemon is transformed
	ld a, PAL_PURPLEMON      ; if yes, use Ditto's palette
	ret nz
	ld a, [hl]
DetermineBackSpritePaletteID_NoStatusCheck: ; 71f9d (1c:5f9d)
	ld [wd11e], a
	and a
	jr z, .idZero
	push bc
